...
import { checkFlowTools } from './native-flow.mjs';
...
try {
  ...
  assert.equal((await rpc('tools/list', {})).result.tools.length, 85);
  ...
  const original = Buffer.alloc(1024); original.set([0xb8, 42, 0, 0, 0, 0xc3]); original.write('GUI_SYNTHETIC', 32);
  for (let i = 0; i < 8; i++) original.writeUInt16LE(100 + i * 10, 128 + i * 2);
  populateResearchBytes(original);const fixture=researchFixture(original);
  ...
  assert.deepEqual((await call('read_bytes', { ...firstIdentity, address: 'ram:00400000', count: original.length })).bytes, [...original]);
  ...
  await call('create_function',{...firstIdentity,address:'ram:00400000',name:'answer_42'});
  await defineResearchFixture(call,firstIdentity);
  ...
  await checkResearchTools(call,firstIdentity,fixture);
  await checkFlowTools(call,firstIdentity,fixture);
  await checkTypeTools(call,firstIdentity,fixture);
  await checkUtilityTools(call,firstIdentity,fixture);
  await call('save_program',firstIdentity);
  ...
  assert.deepEqual((await call('read_bytes', { expected_program_id: second.id, address: 'ram:00400000', count: modified.length })).bytes, [...modified]);
  await call('create_instructions',{expected_program_id:second.id,address:'ram:00400000',length:6});
  await call('create_function',{expected_program_id:second.id,address:'ram:00400000',name:'modified_answer'});
  ...
  await checkSavedComparisons(call,{expected_program_id:selected.id},'/Modified',sha(modified),original);
  await checkTypePersistence(call,{expected_program_id:selected.id},fixture);
  await checkUtilityPersistence(call,{expected_program_id:selected.id},fixture);

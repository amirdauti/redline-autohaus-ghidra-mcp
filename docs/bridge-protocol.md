...
The MCP registry exposes 85 typed operations. The original operations below remain compatible. Expanded native contracts are documented in [research tools](research-tools.md), [decompiler/data-flow tools](flow-tools.md), [type editing](type-tools.md), and [memory/annotation tools](utility-tools.md). These 46 additional commands use the same identity, analysis-busy, transaction, result-validation and uncertain-outcome rules. MCP `tools/list` supplies their strict input schemas.
